...
import com.agrotool.beans.EnsayoMapa;
import com.agrotool.beans.FichaEnsayo;
...
public class RestEnsayosController {
	...
	private void procesarRedDMMAS(Iterator ite, FichaEnsayo f, List<EnsayoMapa> ensayos, String ensayoActual, boolean ensayoCompleto, EnsayoMapa enM)  {
		...
		while (ite.hasNext()) {
			...
			/*
			 * if ("DMMAS".equals(ensayo.getTipode_red().trim()) &&
			 * !ensayoActual.equals(ensayo.getEnsayo()) ) { ensayoActual =
			 * ensayo.getEnsayo(); enM = new EnsayoMapa(); f = new FichaEnsayo();
			 * ensayoCompleto = false; } if ("DMMAS".equals(ensayo.getTipode_red().trim())
			 * && ensayoActual.equals(ensayo.getEnsayo())) { if (f.getAtributos().size()==0)
			 ...
			 * enM.getEnsayo().setTipoDeREd(ensayo.getTipode_red()); // Armamos la ficha del
			 * ensayo enM.getFichaEnsayo().getAtributos().put("RED",
			 * ensayo.getTipode_red()); enM.getFichaEnsayo().getAtributos().put("Localidad",
			 * ensayo.getLocalidad());
			 * enM.getFichaEnsayo().getAtributos().put("Micro Region",
			 * ensayo.getMicroRegion()); enM.getFichaEnsayo().getAtributos().put("Campañia",
			 * ensayo.getCampania()); enM.getFichaEnsayo().getAtributos().put("Firma",
			 * ensayo.getFirma());
			 * 
			 * enM.getFichaEnsayo().getAtributos().put("Fecha de Siembra",
			 * ensayo.getFechaSiembra());
			 * 
			 * enM.getFichaEnsayo().getAtributos().put("Antecesor", ensayo.getAntecesor());
			 * enM.getFichaEnsayo().getAtributos().
			 * put("Rendimiento Promedio del Ensayo (kg/ha)",ensayo.getPromedio_ensayo());
			 * f.getAtributos().put("Producto", "Rendimiento (kg/ha)");
			 * f.getAtributos().put(ensayo.getGenotipo(),
			 * ensayo.getRendimiento().toString()); } if(f.getAtributos().size() < 6 &&
			 * f.getAtributos().size() >= 2) { f.getAtributos().put(ensayo.getGenotipo(),
			 * ensayo.getRendimiento().toString()); } if(f.getAtributos().size() == 6 &&
			 * !ensayoCompleto) { enM.getFichaEnsayo().getAtributos().
			 * put("Ranking 5 Mejores Productos evaluados", f.getAtributos());
			 * ensayos.add(enM); ensayoCompleto = true; } } if(!ite.hasNext()){
			 * enM.getFichaEnsayo().getAtributos().
			 * put("Ranking 5 Mejores Productos evaluados", f.getAtributos());
			 * ensayos.add(enM); }
			 */
